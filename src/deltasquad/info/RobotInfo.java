...
/**
 * The <code>RobotInfo</code> class is used for accessing information about a robot. Whether that information is in the
 * form of constants (e.g., the robot's height, width, etc.) or in the form of accessing the robot's current information
 * (e.g., the robot's current heading, velocity, etc.).
 * 
 * @author Brian Norman
 * @version 0.0.1 beta
 */
public class RobotInfo implements Cloneable, Serializable {
   ...
   public boolean isTeammate(String name) {
      if (this.robot instanceof TeamRobot) {
         ...
         // return teamRobot.isTeammate(name);

         // because isTeammate(name) doesn't work
         boolean isTeammate = false;
         String[] teammates = teamRobot.getTeammates();
         if (teammates != null) {
            for (int i = 0; !isTeammate && i < teammates.length; i++) {
               if (teammates[i].equals(name)) {
                  isTeammate = true;
               }
            }
         }
