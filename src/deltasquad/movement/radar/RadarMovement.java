package deltasquad.movement.radar;

import java.awt.geom.Point2D;

import deltasquad.info.RadarInfo;
...
public class RadarMovement {
   ...
   public final void sweep(RobotData robot, double sweep) {
      if (robot.isDead() || info.getTime() - robot.getTime() > 10) {
    ...
    * 
    * @param robot
    *           - the targeted <code>{@link robocode.Robot Robot}</code> to sweep.
    * @param sweep
    *           - the angle past the <code>{@link robocode.Robot Robot}</code> the radar will sweep.
    */
