...
import deltasquad.data.gravity.GravityObject;
...
import deltasquad.segmentation.Segmentable;
...
public class RobotData implements Cloneable, Serializable, Segmentable, Printable, Drawable, GravityObject {
   ...
   public RobotData(final Robot myRobot) {
      init(myRobot.getName(), myRobot.getX(), myRobot.getY(), myRobot.getEnergy(), myRobot.getHeading(),
            myRobot.getVelocity(), myRobot.getTime());
   }
   ...
   public RobotData(final RobotData robot) {
      init(robot.getName(), robot.getAlias(), robot.getX(), robot.getY(), robot.getEnergy(), robot.getDeltaEnergy(),
            robot.getHeading(), robot.getDeltaHeading(), robot.getAvgDeltaHeading(), robot.getVelocity(),
            robot.getDeltaVelocity(), robot.getAvgVelocity(), robot.getAvgDeltaVelocity(), robot.getTime());
   }
   ...
   /**
    * Updates the robot when it was previously dead. Used only once at the beginning of each round. Updates the robot so
   ...
   public void update(final Robot myRobot) {
      if ((myRobot.getTime() - time) < 0 && isDead()) {
         updateFromDeath(myRobot.getX(), myRobot.getY(), myRobot.getEnergy(), myRobot.getHeading(),
               myRobot.getVelocity(), myRobot.getTime());
      } else if ((myRobot.getTime() - time) > 0 && !isDead()) {
